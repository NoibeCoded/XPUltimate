...
### New version support
XpBottles 2.0 uses a simplified architecture with version-specific handlers instead of separate modules.

1. Create a new handler class in `api/src/main/java/me/codedred/xpbottles/platform/handlers/`
2. Name it `Handler_<version>` (e.g., `Handler_1_21_1.java`)
3. Extend `GenericPlatformHandler` and override methods as needed
4. Update `MinecraftVersion.java` enum if adding a new major version
5. Run `./gradlew build`
...
2. Run `./gradlew build`
3. The final jar will be in `api/build/libs/`
